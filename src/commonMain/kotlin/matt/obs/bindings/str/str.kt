...
operator fun ObsS.plus(other: String): ObsS =
    binding {
        it + other
    }
...
infix fun ObsS.eqIgnoreCase(other: ObsS) =
    binding(other) {
        it loweq other.value
    }

/*not sure whether to use out or in generic here*/
infix fun ObsS.eqIgnoreCase(other: String) =
    binding {
        it loweq other
    }


operator fun ObsS.get(index: Int): ObsVal<Char?> =
    binding(this) {
...
operator fun ObsS.get(index: ObsVal<I>): ObsVal<Char?> =
    binding(this, index) {
...
operator fun ObsS.get(start: Int, end: Int): ObsS =
    binding(this) {
        value.subSequence(start, end).toString()
    }

operator fun ObsS.get(start: ObsVal<I>, end: Int): ObsS =
    binding(this, start) {
        value.subSequence(start.value, end).toString()
    }

operator fun ObsS.get(start: Int, end: ObsVal<I>): ObsS =
    binding(this, end) {
        value.subSequence(start, end.value).toString()
    }

operator fun ObsS.get(start: ObsVal<I>, end: ObsVal<I>): ObsS =
    binding(this, start, end) {
